...
def create_image(system_info):
    ...
    draw.text((10, 160), f"Network Sent: {system_info['network']['bytes_sent'] / (1024 * 1024):.2f} MB", font=font, fill=text_color)
    draw.text((10, 180), f"Network Received: {system_info['network']['bytes_recv'] / (1024 * 1024):.2f} MB", font=font, fill=text_color)
    draw.text((10, 210), "Top Processes:", font=font, fill=text_color)
    for i, proc in enumerate(system_info['top_processes']):
        draw.text((10, 230 + i*20), f"{proc['name']} (PID {proc['pid']}) - CPU: {proc['cpu_percent']}%, MEM: {proc['memory_percent']:.2f}%", font=font, fill=text_color)
    return image
